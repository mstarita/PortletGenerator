 ...
 * @author tomyeh
 */
public class FullDHtmlLayoutPortlet extends GenericPortlet {
	
	public static String PORTLET_PREFS_ATTR_KEY = "portletPrefs";
	public static String LOCALE_ATTR_KEY = "locale";
	public static String LABEL_LOADER_ATTR_KEY = "labelLoader";
	
	private static final Log log = Log.lookup(FullDHtmlLayoutPortlet.class);

	/** The parameter or attribute to specify the path of view the ZUML page. */
	private static final String ATTR_VIEW_PAGE = "zk_view_page";
	/** The parameter or attribute to specify the path of view the richlet. */
	private static final String ATTR_VIEW_RICHLET = "zk_view_richlet";
	/** The default page. */
	private String _defpage;
	
	/** The parameter or attribute to specify the path of the ZUML page. */
	private static final String ATTR_EDIT_PAGE = "zk_edit_page";
	/** The parameter or attribute to specify the path of the richlet. */
	private static final String ATTR_EDIT_RICHLET = "zk_edit_richlet";
	/** The default page. */
	private String _defeditpage;
	
	public void init() throws PortletException {
		_defpage = getPortletConfig().getInitParameter(ATTR_VIEW_PAGE);
		_defeditpage = getPortletConfig().getInitParameter(ATTR_EDIT_PAGE);
	}
	...
	throws PortletException, IOException {
		...
		String path = request.getParameter(ATTR_VIEW_PAGE);
		if (path == null) {
			path = (String)request.getAttribute(ATTR_VIEW_PAGE);
			if (path == null) {
				PortletPreferences prefs = request.getPreferences();
				path = prefs.getValue(ATTR_VIEW_PAGE, null);
				if (path == null) {
					path = request.getParameter(ATTR_VIEW_RICHLET);
					bRichlet = path != null;
					if (!bRichlet) {
						path = (String)request.getAttribute(ATTR_VIEW_RICHLET);
						bRichlet = path != null;
						if (!bRichlet) {
							path = prefs.getValue(ATTR_VIEW_RICHLET, null);
							bRichlet = path != null;
							if (!bRichlet)
								path = _defpage;
						}
	...
	throws PortletException, IOException {
		//try parameter first and then attribute
		boolean bRichlet = false;
		String path = request.getParameter(ATTR_EDIT_PAGE);
		if (path == null) {
			path = (String)request.getAttribute(ATTR_EDIT_PAGE);
			if (path == null) {
				PortletPreferences prefs = request.getPreferences();
				path = prefs.getValue(ATTR_EDIT_PAGE, null);
				if (path == null) {
					path = request.getParameter(ATTR_EDIT_RICHLET);
					bRichlet = path != null;
					if (!bRichlet) {
						path = (String)request.getAttribute(ATTR_EDIT_RICHLET);
						bRichlet = path != null;
						if (!bRichlet) {
							path = prefs.getValue(ATTR_EDIT_RICHLET, null);
							bRichlet = path != null;
							if (!bRichlet)
								path = _defeditpage;
						}
...
//		if (log.debugable()) log.debug("Creates from "+path);
		...
		try {
			if (desktop != null) { //recycle
				final Page page = org.zkoss.zk.ui.http.Utils.getMainPage(desktop);
				if (page != null) {
	...
	private static PageRenderPatch getRenderPatch() {
		...
		synchronized (FullDHtmlLayoutPortlet.class) {
			if (_prpatch != null)
				return _prpatch;
